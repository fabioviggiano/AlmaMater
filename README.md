# Portfolio Accademico - Fabio Viggiano

Benvenuti nel mio repository accademico. Questo spazio raccoglie progetti, relazioni, appunti e materiale di studio relativi al mio percorso di **laurea magistrale in Informatica: Curriculum per il management** presso l'**Alma Mater Studiorum - Università di Bologna** .

L'obiettivo è documentare il mio lavoro e condividere risorse che potrebbero essere utili ad altri studenti.

---

## Struttura del Repository

Ogni cartella in questo repository corrisponde a un esame specifico. All'interno di ciascuna cartella, troverete un file `README.md` dedicato che descrive in dettaglio i contenuti, i progetti e le risorse disponibili per quella materia.

## Corsi ed Esami

Di seguito è riportato l'elenco dei corsi documentati in questo repository. Clicca su un nome per accedere direttamente alla cartella corrispondente.

### 📚 Corsi

*   **[Intelligenza Artificiale](./Intelligenza%20Artificiale/)**
    *   *Descrizione:* .

*   **[Natural Language Processing](./Natural%20Language%20Processing/)**
    *   *Descrizione:* .

*   **[Computational Imaging](./Computational%20Imaging/)**
    *   *Descrizione:* .

*   **[Complementi di Basi di Dati](./Complementi%20di%20Basi%20di%20Dati/)**
    *   *Descrizione:* .

*   **[Social Network Analysis](./Social%20Network%20Analysis/)**
    *   *Descrizione:* .

*   **[Didattica dell' Informatica](./Didattica%20dell'%20Informatica/)**
    *   *Descrizione:* .

---

## Contatti

Per domande o feedback, non esitate a contattarmi.

*   **Email:** `fabio.viggiano@studio.unibo.it`
*   **Profilo GitHub:** [fabioviggiano](https://github.com/fabioviggiano)


